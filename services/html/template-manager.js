...
function renderNodes(nodes) {
  ...
  // ── HEAD ─────────────────────────────────────────────────────────────────────
  registry.register('head', () => `<!DOCTYPE html>
  <html lang="pt-BR">
  <head>
    <meta charset="UTF-8">
 ...
 // ── DESTAQUE ────────────────────────────────────────────────────────────────────
  ...
  registry.register('carrossel', (node) => {
    ...
    const items = node.slides.map((slide, i) => {
            let contentHtml = '';
            // 1) string pura
    ...
    const indicators = node.slides.map((_, i) => `
      <li data-target="#${id}" data-slide-to="${i}"${i === 0 ? ' class="active"' : ''}></li>`).join('');
        ...
        <div class="col">
          <div style="display: flex;gap: 30px;justify-content: space-between;">
            ...
            <div class="topico-anterior">
              <span data-link="${escapeUri(node.anterior)}">Tópico Anterior</span>
